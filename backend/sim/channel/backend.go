...
func (b *backend) Sign(addr wallet.Account, params *channel.Params, state *channel.State) ([]byte, error) {
	...
	if b.ChannelID(params) != state.ID {
		return nil, errors.New("Sign invalid parameters")
	}
	...
	// Now write the StateParamFields
	{
		if err := wire.Encode(w, params.ChallengeDuration); err != nil {
			return nil, errors.WithMessage(err, "state challengeDuration encode")
		}

		if params.App == nil {
			log.Panic("App cant be nil")
		}
		if err := params.App.Def().Encode(w); err != nil {
			log.Panic("Could not write to sha256 hasher")
		}
	}
	...
	sig, err := addr.SignData(buff.Bytes())
	if err != nil {
...
func (b *backend) Verify(addr wallet.Address, params *channel.Params, state *channel.State, sig []byte) (bool, error) {
	...
	{
		if err := wire.Encode(w, params.ChallengeDuration); err != nil {
			return false, errors.WithMessage(err, "state challengeDuration encode")
		}
