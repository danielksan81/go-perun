...
func newStateAppSetup(app *StateApp1, rng *rand.Rand, t *testing.T) stateAppSetup {
	...
	vInitParams := *vParams
	vInitState := &channel.State{ID: vInitParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vInitParams), Data: &stateData{stateApp1InitValue}, IsFinal: false}

	ivInitParams := *ivParams
	ivInitState := &channel.State{ID: vInitParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vInitParams), Data: &stateData{stateApp1InitValue + 1}, IsFinal: false}

	vTransParams := *vParams
	vTransFrom := &channel.State{ID: vInitParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vInitParams), Data: &stateData{stateApp1InitValue}, IsFinal: false}
	vTransTo := &channel.State{ID: vInitParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vInitParams), Data: &stateData{stateApp1InitValue / 2}, IsFinal: false}

	ivTransParams := *ivParams
	ivTransFrom := &channel.State{ID: vInitParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vInitParams), Data: &stateData{10}, IsFinal: false}
	ivTransTo := &channel.State{ID: vInitParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vInitParams), Data: &stateData{80}, IsFinal: false}
...
func newActionAppSetup(app *ActionApp1, rng *rand.Rand, t *testing.T) actionAppSetup {
	...
	vActParams := *vParams
	vActState := &channel.State{ID: vActParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vActParams), Data: &stateData{1 << 10}, IsFinal: false}
	vActPart := version%uint64(len(randParams.Parts)) + 1
	vActAction := &app1Action{Mod, 9}

	ivActParams := *ivParams
	ivActState := &channel.State{ID: ivActParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vActParams), Data: &stateData{-10}, IsFinal: false}
	ivActPart := version % uint64(len(randParams.Parts))
	ivActAction := &app1Action{Mod, 11}

	vApplyParams := *vParams
	vApplyState := &channel.State{ID: vApplyParams.ID(), Version: version, Allocation: *newRandomAllocation(rng, &vActParams), Data: &stateData{1 << 10}, IsFinal: false}
	...
	ivApplyState := &channel.State{ID: ivApplyParams.ID(), Version: uint64(rng.Int63n(1 << 30)), Allocation: *newRandomAllocation(rng, &vActParams), Data: &stateData{-10}, IsFinal: false}
